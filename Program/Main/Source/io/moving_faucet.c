...
// FORWARD
#define  FAUCET_FORWARD         SM_ID_1
#define  FAUCET_FORWARD_SPEED   SM_SPEED_1
#define  FAUCET_FORWARD_INIT_STEP_VAL    (-100)
...
void InitMovingFaucet(void)
{
    ...
    // FAUCET - FORWARD
    HAL_InitStepVal( FAUCET_FORWARD );
    HAL_SetCurrentStep( FAUCET_FORWARD, FAUCET_FORWARD_END_STEP_VAL );
    HAL_SetTargetStep( FAUCET_FORWARD, FAUCET_FORWARD_END_STEP_VAL );
    HAL_SetStepSpeed( FAUCET_FORWARD, FAUCET_FORWARD_SPEED );
#endif
}
...
static U8 InitFaucet(void)
{
    ...
    switch( Faucet.InitStep )
    {
        ...
        case INIT_MOVE_ZERO:
            ...
            // FORWARD
            HAL_SetHoldStep( FAUCET_FORWARD, 0 );
            HAL_SetTargetStep( FAUCET_FORWARD, FAUCET_FORWARD_INIT_STEP_VAL );
        ...
        case INIT_WAIT_ZERO:
            if( HAL_IsDoneMoveStep( FAUCET_UP_DOWN ) == TRUE 
                    &&  HAL_IsDoneMoveStep( FAUCET_FORWARD ) == TRUE )
            {
                Faucet.InitStep++;
...
static U8 ReadyFaucet(void)
{
    ...
    switch( Faucet.ReadyStep )
    {
        ...
        case READY_MOVE_ZERO:
            ...
            // FORWARD
            HAL_SetHoldStep( FAUCET_FORWARD, 0 );
            HAL_SetTargetStep( FAUCET_FORWARD, 0 );
...
static U8 MoveFaucet(void)
{
    ...
    {
        case MOVE_INIT:
            if( HAL_GetAdcValue( ANI_FAUCET_FORWARD ) < 300 )
            {
                HAL_SetTargetStep( FAUCET_FORWARD, FAUCET_FORWARD_END_STEP_VAL );
            }
            else
            {
                //OffsetTargetStep( FAUCET_FORWARD, 200, FAUCET_FORWARD_END_STEP_VAL  );
                Faucet.MoveStep = MOVE_UP_DOWN;
                break;
            }
            ...
            // 이동하면서 컵 위치 감지
        case MOVE_FORWARD:
            if( HAL_GetAdcValue( ANI_FAUCET_UP_DOWN ) > 120 )
            {
                OffsetTargetStep( FAUCET_FORWARD, 200, FAUCET_FORWARD_END_STEP_VAL  );
            ...
            else
            {
                ...
                // 완전 이동 완료?
                if( HAL_IsDoneMoveStep( FAUCET_FORWARD ) == TRUE )
                {
                    Faucet.MoveStep++;
                }
            }
        ...
        case MOVE_UP_DOWN_DONE:
            //if( HAL_GetAdcValue( ANI_FAUCET_UP_DOWN ) > 350 )
            if( HAL_GetAdcValue( ANI_FAUCET_UP_DOWN ) > 200 )
            {
                HAL_StopMove( FAUCET_UP_DOWN );
                Faucet.MoveStep++;
            }
